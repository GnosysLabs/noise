...
2. The API creates an opaque upload capability.
3. The client uploads encrypted blocks to managed object storage.
4. The service verifies expected object IDs, hashes, lengths, and completion.
5. The signed encrypted message references the completed opaque object.

Durability comes from managed storage replication, versioning where
appropriate, backups, and lifecycle policies. The object store never receives
plaintext media or its decryption key.
...
- Import every legacy media shard and tombstone.
...
- Use managed object storage for all new encrypted media.
...
2. Choose the initial PostgreSQL, object-storage, queue, and deployment
   providers.
3. Define the signed device-session protocol and minimum server-visible
   membership record.
4. Specify the canonical event and cursor schema.
5. Specify legacy media compatibility and central object resolution.
6. Build the migration verifier before writing the importer.
7. Review this document and explicitly approve any change to the privacy
